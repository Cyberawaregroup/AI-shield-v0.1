...
import { MessageCircle, Shield, AlertTriangle, CheckCircle, Loader2, Send, Bot, User, RefreshCw } from "lucide-react";
...
export default function SecurityAdvisorSimple() {
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!chatInput.trim() || !currentSessionId || isLoadingChat) return;
    ...
    try {
      // Send message to API
      const response = await api.sendChatMessage(currentSessionId, {
        content: inputValue,
        metadata: {
        ...
        <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-white/25 rounded-lg border border-white/40 shadow-lg">
            <div className="flex items-center gap-3 mb-4">
              ...
              {isCreatingSession && (
                <Badge className="bg-yellow-500/20 border-yellow-500/50 text-yellow-300 text-xs">
                  Starting Session...
                </Badge>
              )}
            </div>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto mb-4">
              {chatMessages.length === 0 && !isCreatingSession ? (
            ...
            <form onSubmit={handleChatSubmit} className="flex gap-2">
              <Input
                ...
                disabled={isLoadingChat || !currentSessionId || isCreatingSession}
              />
              <Button
                type="submit"
                disabled={!chatInput.trim() || isLoadingChat || !currentSessionId || isCreatingSession}
