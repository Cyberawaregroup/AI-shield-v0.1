...
from app.models.chatbot import ChatSession, ChatMessage, FraudType, RiskLevel
...
class ChatbotService:
    ...
    def __init__(self, huggingface_service: HuggingFaceService, threat_service: ThreatIntelligenceService):
    ...
    async def generate_initial_response(
        ...
        try:
            ...
            analysis = await self._analyze_message(initial_message, fraud_type, vulnerability_factors)
            ...
            if analysis["risk_level"] == RiskLevel.CRITICAL:
                ...
                response = self.common_responses.get(analysis["fraud_type"], self.common_responses["greeting"])
                escalate = False
    ...
    async def generate_response(
        ...
        try:
            ...
            risk_analysis = await self._analyze_message(user_message, session_context.get("fraud_type"))
            ...
            if self.huggingface_service.is_available():
                ...
                response_content = self._generate_fallback_response(user_message, risk_analysis)
                ai_confidence = 0.5
            ...
            return {
                ...
                "metadata": {
                    ...
                    "escalation_reason": escalation_reason,
                    "ai_model": "gpt-4o" if self.openai_service.is_available() else "fallback"
                },
    ...
    async def escalate_session(
        ...
        try:
            ...
            logger.info(f"Session {session_id} escalated: {reason} (Priority: {priority})")
            return True
    ...
    async def _analyze_message(
        ...
        try:
            ...
            if not fraud_type:
                if any(word in message_lower for word in ["phish", "email", "link", "click"]):
                    fraud_type = FraudType.PHISHING
                elif any(word in message_lower for word in ["romance", "love", "relationship", "dating"]):
                    fraud_type = FraudType.ROMANCE_SCAM
                elif any(word in message_lower for word in ["invest", "money", "profit", "return"]):
                    fraud_type = FraudType.INVESTMENT_SCAM
                elif any(word in message_lower for word in ["tech", "support", "computer", "virus"]):
                    fraud_type = FraudType.TECH_SUPPORT_SCAM
                elif any(word in message_lower for word in ["bank", "account", "card", "payment"]):
            ...
            # High-risk keywords
            high_risk_words = ["urgent", "immediate", "now", "limited time", "act fast", "don't tell anyone"]
            if any(word in message_lower for word in high_risk_words):
                risk_level = RiskLevel.HIGH
            
            # Critical risk indicators
            critical_words = ["bank transfer", "gift cards", "bitcoin", "crypto", "social security"]
            ...
            return {
                ...
                "keywords_found": [word for word in high_risk_words + critical_words if word in message_lower]
            }
    ...
    def _generate_fallback_response(self, message: str, analysis: Dict[str, Any]) -> str:
        """
