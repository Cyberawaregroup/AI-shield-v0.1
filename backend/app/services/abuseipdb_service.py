...
class AbuseIPDBService:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("AbuseIPDB API key not configured. IP reputation checking will be limited.")
    ...
    def _mock_ip_check(self, ip_address: str) -> Dict[str, Any]:
        ...
        if ip_address.startswith("192.168.") or ip_address.startswith("10.") or ip_address.startswith("172."):
            ...
            return {
                ...
                "usage_type": random.choice(["ISP", "Hosting", "Residential", "Mobile"]),
                "source": "mock",
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to report IP {ip_address}: {response.status_code} - {response.text}")
                    return False
    ...
    async def get_blacklist(self, confidence_minimum: int = 90, limit: int = 100) -> list:
        """
        ...
        try:
            if not self.api_key:
                logger.warning("Cannot get blacklist - AbuseIPDB API key not configured")
                return []
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to get blacklist: {response.status_code} - {response.text}")
                    return []
