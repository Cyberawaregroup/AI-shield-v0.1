...
class HuggingFaceService:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("Hugging Face API token not configured. AI features will be limited.")
        else:
            try:
    ...
    async def generate_fraud_advice(
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": self._get_cybersecurity_system_prompt()
                },
                {
                    "role": "user", 
...
Please analyze this situation and provide specific cybersecurity guidance. Focus on immediate protection measures and warning signs to watch for."""
    ...
    def _calculate_confidence(self, risk_analysis: Dict[str, Any], content: str) -> float:
        """
        ...
        if any(keyword in content.lower() for keyword in ["immediately", "urgent", "stop", "danger"]):
            base_confidence += 0.05
        
        if any(keyword in content.lower() for keyword in ["contact", "report", "verify", "check"]):
            base_confidence += 0.05
            
        return min(base_confidence, 1.0)
    
    def _generate_fallback_response(self, user_message: str, risk_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """
...
Respond in JSON format with fraud_type, risk_level, confidence, and warning_signs."""
            ...
            if response and 'choices' in response and len(response['choices']) > 0:
                ...
                try:
                    ...
                    return {
                        "fraud_type": ai_analysis.get("fraud_type", fraud_type or "unknown"),
    ...
    def _basic_ai_analysis(
        ...
        if not fraud_type:
            if any(word in message_lower for word in ["phish", "email", "link", "click", "verify", "account"]):
                fraud_type = "phishing"
            elif any(word in message_lower for word in ["love", "romance", "relationship", "marriage", "boyfriend", "girlfriend"]):
                fraud_type = "romance_scam"
            elif any(word in message_lower for word in ["invest", "money", "profit", "crypto", "bitcoin", "trading"]):
                fraud_type = "investment_scam"
            elif any(word in message_lower for word in ["support", "computer", "virus", "malware", "fix"]):
        ...
        critical_words = ["bank transfer", "gift cards", "bitcoin", "crypto", "wire transfer", "western union"]
        if any(word in message_lower for word in critical_words):
            risk_level = "critical"
            confidence += 0.3
            warning_signs.extend([word for word in critical_words if word in message_lower])
        
        # High-risk indicators
        high_risk_words = ["urgent", "immediate", "now", "limited time", "act fast", "expires soon"]
        if any(word in message_lower for word in high_risk_words):
            if risk_level != "critical":
                risk_level = "high"
            confidence += 0.2
            warning_signs.extend([word for word in high_risk_words if word in message_lower])
        
        # Medium risk indicators
        medium_risk_words = ["free", "winner", "congratulations", "claim", "verify", "suspended"]
        if any(word in message_lower for word in medium_risk_words):
            if risk_level == "low":
                risk_level = "medium"
            confidence += 0.1
            warning_signs.extend([word for word in medium_risk_words if word in message_lower])
