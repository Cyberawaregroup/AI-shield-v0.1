...
class PhishScanService:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("zvelo PhishScan API key not configured. Phishing detection will be limited.")
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"zvelo PhishScan API error: {response.status_code} - {response.text}")
                    return self._mock_url_check(url)
    ...
    def _mock_url_check(self, url: str) -> Dict[str, Any]:
        ...
        phishing_indicators = [
            "bank", "paypal", "amazon", "ebay", "apple", "microsoft", "netflix",
            "login", "verify", "secure", "account", "suspended", "locked"
        ]
        
        # Check for suspicious patterns
        suspicious_patterns = [
            "bank-secure", "paypal-verify", "amazon-account", "ebay-secure",
            "apple-verify", "microsoft-secure", "netflix-account"
        ]
        ...
        if not is_phishing:
            indicator_count = sum(1 for indicator in phishing_indicators if indicator in url_lower)
    ...
    async def check_bulk_urls(self, urls: list) -> list:
        ...
        try:
            if not self.api_key:
                logger.warning("Cannot perform bulk check - zvelo PhishScan API key not configured")
    ...
    async def get_phishing_stats(self) -> Dict[str, Any]:
        ...
        try:
            if not self.api_key:
                logger.warning("Cannot get stats - zvelo PhishScan API key not configured")
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to get phishing stats: {response.status_code} - {response.text}")
                    return self._mock_stats()
