...
from app.models.threat_intelligence import (
    IOC, IOCResponse, IOCCreate, ThreatAlert, ThreatAlertResponse, ThreatAlertCreate,
    BreachExposure, ThreatFeed
)
...
class ThreatIntelligenceService:
    ...
    def __init__(self, hibp_service: HIBPService, abuseipdb_service: AbuseIPDBService, phishscan_service: PhishScanService):
        self.hibp_service = hibp_service
        self.abuseipdb_service = abuseipdb_service
        self.phishscan_service = phishscan_service
    
    async def check_email_breaches(self, email: str, db_session: Session) -> List[Dict[str, Any]]:
    ...
    async def check_ip_reputation(self, ip_address: str, db_session: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            if reputation.get("abuse_confidence_score", 0) > 50:
                ioc = IOC(
                    ...
                    severity="high" if reputation.get("abuse_confidence_score", 0) > 80 else "medium",
                    confidence_score=reputation.get("abuse_confidence_score", 0) / 100.0,
                    source="abuseipdb",
    ...
    async def get_iocs(self, db_session: Session, limit: int = 100) -> List[IOCResponse]:
    ...
    async def create_ioc(self, ioc_data: IOCCreate, db_session: Session) -> Optional[IOCResponse]:
        """Create a new IOC"""
        try:
            ioc = IOC(
    ...
    async def get_threat_alerts(self, db_session: Session, limit: int = 100) -> List[ThreatAlertResponse]:
    ...
    async def create_threat_alert(self, alert_data: ThreatAlertCreate, db_session: Session) -> Optional[ThreatAlertResponse]:
