...
class OpenAIService:
    ...
    def _generate_mock_response(self, user_message: str, risk_analysis: Dict[str, Any]) -> str:
        """
    ...
    async def analyze_message(
        ...
        try:
            ...
            analysis = self._basic_ai_analysis(message, fraud_type, vulnerability_factors)
    ...
    def _basic_ai_analysis(
        ...
        if not fraud_type:
            if any(word in message_lower for word in ["phish", "email", "link"]):
                fraud_type = "phishing"
            elif any(word in message_lower for word in ["love", "romance", "relationship"]):
        ...
        return {
            ...
            "keywords": [word for word in high_risk_words + critical_words if word in message_lower]
        }
